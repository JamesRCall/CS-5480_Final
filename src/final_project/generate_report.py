...
def save_model_summary(metrics: pd.DataFrame, output_dir: Path) -> None:
    ...
    plot_data.plot.bar(ax=ax, rot=0)
    y_min, y_max = _get_plot_limits(plot_data.stack(), min_margin=0.05, max_margin=0.08, cap_max=1.03)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "model_performance_summary.png")
    plt.close()
